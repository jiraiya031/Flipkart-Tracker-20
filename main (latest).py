...
def create_graph_from_excel():
    ...
    for row in worksheet.iter_rows(min_row=2, values_only=True):
        price1_list.append(row[0])
        price2_list.append(row[1])
        time_list.append(row[2])

    # Convert the time string to datetime objects
    ...
    # Plot the data
    ax.plot(time_list, price1_list, label='Price1')
    ax.plot(time_list, price2_list, label='Price2')
